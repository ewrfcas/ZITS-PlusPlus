...
class FullyConnectedLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.activation == 'linear' and b is not None:
            ...
            x = x.matmul(w.t())
            out = x + b.reshape([-1 if i == x.ndim-1 else 1 for i in range(x.ndim)]).to(x.dtype)
        else:
            x = x.matmul(w.t())
            out = bias_act.bias_act(x, b.to(x.dtype), act=self.activation, dim=x.ndim-1)
        return out
...
class Conv2dLayer(nn.Module):
    ...
    def forward(self, x, gain=1):
        ...
        out = bias_act.bias_act(x, self.bias.to(x.dtype) if self.bias is not None else self.bias,
                                act=self.activation, gain=act_gain, clamp=act_clamp)
        return out
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, x, style):
        ...
        x = conv2d_resample.conv2d_resample(x=x, w=weight.to(x.dtype), f=self.resample_filter, up=self.up, down=self.down,
...
class ToRGB(torch.nn.Module):
    ...
    def forward(self, x, style, skip=None):
        x = self.conv(x, style)
        out = bias_act.bias_act(x, self.bias.to(x.dtype), clamp=self.conv_clamp)
...
class MappingNet(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False, sym=False):
...
@persistence.persistent_class
class DisNoDownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, activation):  # res = 2, ..., resolution_log2
        super().__init__()
        self.conv0 = Conv2dLayer(in_channels=in_channels,
                                 out_channels=in_channels,
